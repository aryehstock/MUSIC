...
```{r, eval= FALSE}
NHI_rec_ac |> 
  gtsave(filename = "output_tables/nhi_r_a.docx")

NHI_rec_tc |> 
  gtsave(filename = "output_tables/nhi_r_t.docx")

NHI_rec_dc |> 
  gtsave(filename = "output_tables/nhi_r_d.docx")
```
...
# Max NHI in Rectum plus DC vs Max NHI in AC, TC
```{r}
df_NHI_re_dc <- df_NHI |> 
  mutate(
    across(3:32, as.integer)
...
```{r, eval = FALSE}
NHI_rd_ac |> 
  gtsave(filename = "output_tables/nhi_rd_ac.docx")

NHI_rd_tc |> 
  gtsave(filename = "output_tables/nhi_rd_tc.docx")
```
...
```{r, eval = FALSE}
NHI_rd_at_per |> 
  gtsave(filename = "output_tables/nhi_rd_at.png")
```

```{r, eval = FALSE}
NHI_rd_at_per |> 
  gtsave(filename = "output_tables/nhi_rd_at.docx")
```

```{r, eval = FALSE}
NHI_rd_tc_per |> 
  gtsave(filename = "output_tables/nhi_rd_t_per.docx")
```
